...
export default class Jira {
    ...
    if (config.jira.api.host) {
      if (config.jira.api.oauth) {
        ...
        if (!token && typeof password !== "undefined") {
          console.warn(
            "WARNING: Jira password is deprecated. Use an API token instead."
          );
          token = password;
        }
        if (!email && typeof username !== "undefined") {
          console.warn(
            "WARNING: Jira username is deprecated for API authentication. Use user email instead."
          );
          email = username;
        }
  ...
  async generate(commitLogs, releaseVersion) {
    ...
    try {
      const promises = commitLogs.map(commit => {
        return this.findJiraInCommit(commit, releaseVersion).then(log => {
          logs.push(log);
        });
      });
  ...
  includeTicket(ticket) {
    ...
    if (
      Array.isArray(this.config.jira.includeIssueTypes) &&
      this.config.jira.includeIssueTypes.length
    ) {
